...
class ModificationStop {
     ...
     */
    private ModificationStop (Coordinate c, String id, boolean autoGenerated, int hopTime) {
        this.coordinate = c;
        this.id = id;
    ...
    static List<ModificationStop> getStopsFromSegments (List<Segment> segments, int[] segmentSpeedsKph) {
        ...
        if (firstSegment.stopAtStart) {
            stops.add(new ModificationStop(firstSegment.geometry.getCoordinates()[0],
                    firstSegment.fromStopId, false, -1));
        }
        ...
        int hopTime = 0; // accumulated hopTime since last stop
        for (int i = 0; i < segments.size(); i++) {
            ...
            double metersFromSegmentStart = metersFromPatternStart;
            ...
            for (int j = 1; j < coords.length; j++) {
                ...
                double metersAtEndOfSegment = metersFromSegmentStart + lineSegmentMeters;
                ...
                if (autoCreateStops) {
                    ...
                    while (metersToLastStop + stopSpacing < metersAtEndOfSegment) {
                        ...
                        double accumulatedMeters = frac * lineSegmentMeters;
                        metersToLastStop = metersFromPatternStart + accumulatedMeters;

                        // Add the accumulated hop time which could contain time from a previous segment.
                        hopTime += (int) (accumulatedMeters / segmentSpeedMps);

                        // Add the auto-created stop to the stack, flagging it as an auto-created one.
                        stops.add(new ModificationStop(c, null, true, hopTime));

                        // Restart hopTime accumulation
                        hopTime = 0;
                    }

                    // Accumulate the hop time from the last auto generated stop to the end of the segment
                    hopTime += (int) ((metersAtEndOfSegment - metersToLastStop) / segmentSpeedMps);
                } else {
                    // Accumulate hop time for the entire segment
            ...
            if (segment.stopAtEnd) {
                ...
                if (autoCreateStops && stops.size() > 1) {
                    ...
                    if (lastStop.autoGenerated && spacingPercentage < MIN_SPACING_PERCENTAGE) {
                        // Remove the stop and add back the accumulated hop time
                ...
                stops.add(new ModificationStop(endCoord, segment.toStopId, false, hopTime));

                // restart the auto-generated stop spacing
                metersToLastStop = metersFromPatternStart;

                // Restart the hopTime accumulation
                hopTime = 0;
            }
        }
